...
/**
 * A pattern branch can produce more than one possible end position.
 * The matcher keeps them unique so alternation/repetition cannot grow duplicate
 * states indefinitely.
 */
using Positions = std::vector<std::size_t>;

void append_unique(Positions& target, std::size_t value) {
    if (std::find(target.begin(), target.end(), value) ==
        target.end()) {
...
Positions match_repetition(
    const RepetitionPatternNode& repetition,
    std::string_view text,
    std::size_t position) {
    Positions result;
    Positions frontier;
    ...
    if (repetition.type() ==
        ...
        frontier =
            match_from(repetition.operand(), text, position);
        append_unique(result, frontier);
    }

    // Breadth-style expansion explores every reachable end position. Matches
    // that consume zero bytes are ignored as next-frontier states so a
    // zero-length operand cannot cause an infinite repetition loop.
    while (!frontier.empty()) {
        Positions next_frontier;
        for (const auto current : frontier) {
            const auto ends =
                match_from(repetition.operand(), text, current);
            for (const auto end : ends) {
                if (end == current) {
                    continue;
                }
                if (std::find(result.begin(), result.end(), end) ==
                    result.end()) {
...
Positions match_from(const PatternNode& node,
    ...
    case AstNodeKind::PatternLiteral: {
        const auto& literal =
            static_cast<const LiteralPatternNode&>(node);
        if (position < text.size() &&
            text[position] == literal.value()) {
            return {position + 1};
        }
        return {};
    }
    ...
    case AstNodeKind::PatternAnchor: {
        const auto& anchor =
            static_cast<const AnchorPatternNode&>(node);
        if (anchor.type() ==
            AnchorPatternNode::Type::StartOfLine) {
            return position == 0
                ? Positions{position}
                : Positions{};
        }
    ...
    case AstNodeKind::PatternSequence: {
        const auto& sequence =
            static_cast<const SequencePatternNode&>(node);
        ...
        for (const auto& element : sequence.elements()) {
            ...
            for (const auto current : positions) {
                append_unique(
                    next,
                    match_from(*element, text, current));
            }
    ...
    case AstNodeKind::PatternAlternation: {
        const auto& alternation =
            static_cast<const AlternationPatternNode&>(node);
        Positions result;
        for (const auto& alternative :
             alternation.alternatives()) {
            append_unique(
                result,
                match_from(*alternative, text, position));
        }
        return result;
    }
    ...
    case AstNodeKind::PatternRepetition:
        return match_repetition(
            static_cast<const RepetitionPatternNode&>(node),
            text,
            position);
    ...
    case AstNodeKind::PatternGroup: {
        const auto& group =
            static_cast<const GroupPatternNode&>(node);
        return match_from(
            group.expression(), text, position);
    }

    default:
        // PatternMatcher is intentionally permissive internally: an unknown
        // node kind simply does not match. Parser/runtime boundaries prevent
        // command/address AST nodes from normally reaching this function.
        return {};
    }
}
...
std::optional<PatternMatch> PatternMatcher::find(
    ...
    // Earliest matching start wins. If that start yields several legal end
    // positions, choose the longest one.
    for (std::size_t start = start_offset;
         start <= text.size();
         ++start) {
